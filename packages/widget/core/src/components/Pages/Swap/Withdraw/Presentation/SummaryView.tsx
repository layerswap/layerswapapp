...
export type SwapInfoProps = Omit<SwapResponse, 'quote' | 'swap'> & {
    swap: SwapBasicData;
    quote: Quote;
...
const Summary: FC<SwapInfoProps> = (props) => {
    ...
    const {
        ...
        destination_token: destinationCurrency,
        source_network: from,
    ...
    const truncatedRefuelAmount =
        nativeCurrency && !!refuel
            ? truncateDecimals(refuel.amount, nativeCurrency?.precision)
            : null;
    const refuelAmountInUsd =
        nativeCurrency &&
        (
            (nativeCurrency?.price_in_usd || 1) *
            (Number(truncatedRefuelAmount) || 0)
        ).toFixed(2);
                ...
                <SwapRow
                    ...
                    primary={
                        requestedAmount ? (
                            isUsdMode ? (
                                <NumFlowWithFallback
                                    value={Number(requestedAmountInUsd) || 0}
                                    prefix="$"
                                    trend={0}
                                />
                            ) : (
                        ...
                        receiveAmount ? (
                            isUsdMode ? (
                                ...
                                <NumFlowWithFallback
                                    ...
                                    suffix={` ${destinationCurrency.asset}`}
                                    trend={0}
                                    format={{
                ...
                {!!refuel != undefined && nativeCurrency ? (
                    <div className="flex items-center justify-between w-full ">
                        ...
                        <div className="flex flex-col items-end">
                            ...
                            <p className="text-secondary-text text-sm flex justify-end">
                                ${refuelAmountInUsd}
                            </p>
                        </div>
                    </div>
        ...
        <div className="w-full flex items-center gap-4 text-left md:text-base min-w-0">
            ...
            <div className="flex flex-col grow min-w-0">
                <div className="flex items-center gap-2 h-8">
                    <p className="text-primary-text text-xl leading-6 font-normal grow truncate min-w-0">
                        {token.asset}
                    </p>
                    <div className="text-primary-text text-xl leading-6 font-normal flex items-center justify-end shrink-0 whitespace-nowrap">
                        {primary}
                    </div>
                </div>
                <div className="flex items-center gap-2 h-5">
                    ...
                    <div className="text-secondary-text text-sm flex items-center justify-end gap-1 font-medium shrink-0 whitespace-nowrap">
                        {secondary}
                    </div>
                </div>
            </div>
