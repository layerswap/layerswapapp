...
export function RPCUnhealthyView({
    RPCUrl,
    isSuggestingRpc,
    rpcAddStatus,
    isCopied,
    onCopy,
    handleAddRpc,
}: MessageContentProps & { handleAddRpc?: () => void }) {
    return (
...
const MessageContent: FC<MessageContentProps> = ({
    ...
    const getMessage = () => {
        if (isSuggestingRpc) {
            return (
                <p className="text-secondary-text text-sm leading-[18px]">
        ...
        if (rpcAddStatus === 'success') {
            return (
                <p className="text-secondary-text text-sm leading-[18px]">
                    RPC URL added! Please switch to the new RPC in your wallet
                    settings.
                </p>
            );
        }
        if (rpcAddStatus === 'error') {
            return (
                <div
                    className="space-y-2 text-secondary-text text-sm leading-[18px]"
                    aria-live="polite"
                >
                    <p>
                        We couldn’t add the RPC URL automatically. Copy it below
                        and update it in your wallet’s network settings.
                    </p>
                    <button
                        type="button"
                        onClick={onCopy}
        ...
        return (
            <p className="text-secondary-text text-sm leading-[18px] space-x-1">
                <span>Update your RPC URL manually or add our</span>
                <Tooltip>
                    <TooltipTrigger onClick={onCopy}>
