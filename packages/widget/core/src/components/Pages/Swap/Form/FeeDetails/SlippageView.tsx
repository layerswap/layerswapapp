...
export const SlippageView = ({
    quoteData,
    slippage,
    autoSlippage = true,
    setSlippage = () => {},
    setAutoSlippage = () => {},
    disableEditingBackground,
}: SlippageProps) => {
    ...
    const isHighSlippage =
        currentSlippagePercent > HIGH_SLIPPAGE_THRESHOLD_PERCENT;
    ...
    return (
        <div
            ref={ref}
            className={clsx(
                'flex items-center w-full justify-between gap-1 text-sm py-1',
                disableEditingBackground ? 'px-3' : 'px-2',
                {
                    'bg-secondary-700 rounded-xl':
                        editingSlippage && !disableEditingBackground,
                },
            )}
        >
            <div className="inline-flex items-center text-left py-2">
                <label className="flex items-center gap-1">
                    ...
                    <Tooltip openOnClick>
                        ...
                        <TooltipContent
                            className="pointer-events-none w-80 grow p-2 border-none! bg-secondary-300! text-xs rounded-xl"
                            side="top"
                            align="start"
                            alignOffset={-30}
                        >
                            <p>
                                {isHighSlippage
                                    ? 'High slippage increases the risk of receiving significantly less than the quoted amount.'
                                    : 'Your transaction will be refunded if the price moves more than the slippage percentage.'}
                            </p>
                        </TooltipContent>
                    </Tooltip>
                </label>
            ...
            {!editingSlippage && (
                <div className="text-right flex items-center gap-1">
                    {!slippage && (
                        <div className="text-secondary-text">(Auto)</div>
                    )}
                    <span
                        className={clsx(
                            isHighSlippage
                ...
                <div className="flex items-center gap-1">
                    {!autoSlippage && (
                        <span className="flex items-center gap-1 max-sm:hidden">
                            ...
                            />
                            <QuickAction value={1} setSlippage={setSlippage} />
                            <QuickAction
                                value={2.5}
                                setSlippage={setSlippage}
                            />
                    ...
                    {!editingCustomSlippage && (
                        <div
                            className={clsx(
                                'flex items-center gap-1 text-sm px-2 h-8 border border-secondary-300 rounded-lg font-normal leading-4 cursor-pointer',
                                isHighSlippage &&
                                    'shadow-[inset_0_0_0_1px] shadow-warning-foreground',
                            )}
                            onClick={() => {
                    ...
                    <div
                        ...
                        className={clsx(
                            ...
                            autoSlippage
                                ? 'bg-secondary-300 border-secondary-100'
                                : 'bg-secondary-500 border-transparent',
                        )}
                    >
                        Auto
                    </div>
...
const QuickAction = ({ value, setSlippage }: QuickActionProps) => {
...
const SlippageInput = forwardRef<HTMLInputElement, SlippageInputProps>(
    function SlippageInput(
        { valueDecimal, onDebouncedChange, onEditing },
        ref,
    ) {
        const [localPercent, setLocalPercent] = useState<number | undefined>(
            valueDecimal !== undefined ? valueDecimal * 100 : undefined,
        );
        ...
        useEffect(() => {
            const newPercent =
                valueDecimal !== undefined
                    ? Math.round(valueDecimal * 10000) / 100
                    : undefined;
        ...
        const invalid =
            localPercent !== undefined &&
            (localPercent < 0.1 || localPercent > 5);
        const isHighSlippage =
            localPercent !== undefined &&
            localPercent > HIGH_SLIPPAGE_THRESHOLD_PERCENT;
        ...
        useEffect(() => {
            const t = setTimeout(() => {
                ...
                onDebouncedChange(
                    localPercent !== undefined
                        ? Math.round(localPercent * 100) / 10000
                        : undefined,
                );
            }, 300);
                    ...
                    <PopoverTrigger asChild>
                        ...
                        >
                            <input
                                ...
                                title=""
                                className={clsx(
                                    'w-8 bg-transparent border-none outline-none focus:outline-none focus-visible:outline-none ring-0 focus:ring-0 focus-visible:ring-0 focus:border-transparent focus:shadow-none text-base leading-3.5 p-0 text-right',
                                    isHighSlippage
                                        ? 'text-warning-foreground'
                                        : 'text-primary-text',
                                )}
