...
`walletProviders` is the union `(WalletProvider | WalletWrapper | WalletProviderDescriptor)[]` (all three from `@layerswap/ui-kit/types`). Only `WalletProvider` carries resolver fields (`transferProvider`, `balanceProvider`, `gaslessProvider`, `extendedRouteProvider`, etc.); descriptors are lazy stubs and wrappers have no resolvers.
