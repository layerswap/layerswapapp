...
const SelectNetwork = forwardRef(({ direction, label }: Props, ref: any) => {
    ...
    const { lockNetwork, destNetwork, sourceExchangeName } = useQueryState()
    ...
    if (direction === "from" ? (swapType === SwapType.OnRamp) : (swapType === SwapType.OffRamp)) {
        menuItems = generateExchangeMenuItems({ exchanges, values, networks, resource_storage_url, sourceExchangeName });
