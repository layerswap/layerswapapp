...
const SwapForm: FC<Props> = ({ partner, isPartnerWallet }) => {
    ...
    const sourceRoutesEndpoint = `/sources?include_unmatched=true&include_swaps=true&include_unavailable=true&destination_network=${source?.name}&destination_token=${fromCurrency?.symbol}`
    ...
    const { data: sourceRoutes, isLoading: sourceLoading } = useSWR<ApiResponse<RouteNetwork[]>>((source && fromCurrency) ?
        sourceRoutesEndpoint : `/sources?include_unmatched=true&include_swaps=true&include_unavailable=true`, layerswapApiClient.fetcher, { keepPreviousData: true })

    const { data: destinationRoutes, isLoading: destinationLoading } = useSWR<ApiResponse<RouteNetwork[]>>((destination && toCurrency) ?
        destinationRoutesEndpoint : `/destinations?include_unmatched=true&include_swaps=true&include_unavailable=true`, layerswapApiClient.fetcher, { keepPreviousData: true })
