...
const SwapForm: FC<Props> = ({ partner, isPartnerWallet }) => {
    ...
    const walletBalance = balances?.find(b => b?.network === values?.from?.internal_name && b?.token === values?.fromCurrency?.asset)
    const networkGas = values.from?.internal_name ?
        gases?.[values.from?.internal_name]?.find(g => g.token === values?.fromCurrency?.asset)
        : null

    useEffect(() => {
        address && values.from && values.fromCurrency && getGas(values.from, values.fromCurrency, values.destination_address || address)
            ...
            <Form className={`h-full ${(isSubmitting) ? 'pointer-events-none' : 'pointer-events-auto'}`} >
                <Widget.Content>
                    <div className='flex-col relative flex justify-between w-full space-y-4 mb-3.5 leading-4'>
                        ...
                        {!query?.hideFrom && !query?.hideTo && <button type="button" disabled={valuesSwapperDisabled} onClick={valuesSwapper} className={`${sourceLoading || destinationLoading ? "" : "border-4"} absolute right-[calc(50%-16px)] top-[74px] z-10 border-secondary-900 bg-secondary-900 rounded-full disabled:cursor-not-allowed hover:text-primary disabled:text-secondary-text duration-200 transition`}>
