...
import RefuelBoxComponent from "./RefuelBoxComponent";
...
const SwapForm: FC<Props> = ({ partner, isPartnerWallet }) => {
  ...
  const minAllowedAmount = CalculateMinAllowedAmount(
    values,
    settings.networks,
    settings.currencies
  );
  ...
  const lockAddress =
    ...
    (query.appName !== "imxMarketplace" || settings.validSignatureisPresent);
  ...
  useEffect(() => {
    if (
      ...
      (!source ||
        !asset ||
        !GetNetworkCurrency(source, asset)?.is_refuel_enabled)
    ) {
  ...
  useEffect(() => {
    if (
      ((previouslySelectedDestination.current &&
        destination?.isExchange !=
          previouslySelectedDestination.current?.isExchange) ||
        (destination?.isExchange &&
          previouslySelectedDestination.current?.isExchange &&
  ...
  useEffect(() => {
    const filteredSourceLayers = FilterSourceLayers(
      settings.layers,
      source,
      lockedCurrency
    );
    const filteredDestinationLayers = FilterDestinationLayers(
      settings.layers,
      destination,
      lockedCurrency
    );

    const sourceCanBeSwapped = filteredDestinationLayers.some(
      (l) => l.internal_name === source?.internal_name
    );
    const destinationCanBeSwapped = filteredSourceLayers.some(
      (l) => l.internal_name === destination?.internal_name
    );
  ...
  const destinationNetwork = GetDefaultNetwork(
    destination,
    values?.currency?.asset
  );
  const destination_native_currency =
    !destination?.isExchange && destinationNetwork?.native_currency;

  const averageTimeString =
    (values?.to?.isExchange === true
      ? values?.to?.assets?.find(
          (a) => a?.asset === values?.currency?.asset && a?.is_default
        )?.network?.average_completion_time
  ...
  const shouldRenderRefuelBox = (destination, asset, query) => {
    return (
      destination &&
      asset &&
      !destination.isExchange &&
      GetNetworkCurrency(destination, asset)?.is_refuel_enabled &&
      !query?.hideRefuel
    );
  };
        ...
        >
          <Widget.Content>
                  ...
                  >
                    <ArrowUpDown
                      className={classNames(
                        valuesSwapperDisabled && "opacity-50",
                        "w-8 h-auto p-1 bg-secondary-900 border-2 border-secondary-500 rounded-full disabled:opacity-30"
                      )}
                    />
                  </motion.div>
                </button>
            ...
            {!hideAddress ? (
              <div className="w-full mb-3.5 leading-4">
                <label
                  ...
                  className="block font-semibold text-secondary-text text-sm"
                >
            ...
            <div className="w-full">
              {shouldRenderRefuelBox(destination, asset, query) && (
                <RefuelBoxComponent
                  values={values}
                  handleConfirmToggleChange={handleConfirmToggleChange}
                  destination_native_currency={destination_native_currency}
                />
              )}
              {
                //TODO refactor
                destination &&
                  asset &&
                  GetNetworkCurrency(destination, asset)?.status ==
                    ...
                    <WarningMessage messageType="warning" className="mt-4">
                      <span className="font-normal">
                        ...
                        <span>{values?.currency?.asset}</span> <span>to</span>{" "}
                ...
                averageTimeInMinutes > 30 && (
                  <WarningMessage messageType="warning" className="mt-4">
                    <span className="font-normal">
                      ...
                      <span>
                        network congestion. Transactions can take up to 1 hour.
                      </span>
                    </span>
                  </WarningMessage>
                )}
...
const AddressButton: FC<AddressButtonProps> = ({
  ...
  return (
    <button
      ...
      className="flex rounded-lg space-x-3 items-center cursor-pointer shadow-sm mt-1.5 text-primary-text-placeholder bg-secondary-700 border-secondary-500 border disabled:cursor-not-allowed h-12 leading-4 focus:ring-primary focus:border-primary font-semibold w-full px-3.5 py-3"
    >
