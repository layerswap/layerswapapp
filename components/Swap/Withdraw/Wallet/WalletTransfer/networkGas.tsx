...
const NetworkGas: FC<NetworkGasProps> = ({ selected_currency, network }) => {
    ...
    const networkGas = network.internal_name ?
        gases?.[network.internal_name]?.find(g => g?.token === selected_currency.symbol)?.gas : null
    ...
    return <div className="flex flex-row items-center gap-1 w-full text-sm px-1 mb-2.5 justify-end">
        ...
        <div className="text-right flex items-center gap-1">
            {isGasLoading ? <div className='h-[10px] w-10 bg-gray-500 rounded-sm animate-pulse' /> : estimatedGas} <span>{network?.assets.find(a => a.is_native)?.symbol ?? selected_currency.symbol}</span>
        </div>
    </div>
}
