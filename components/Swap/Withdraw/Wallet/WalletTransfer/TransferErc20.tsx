...
const TransferErc20Button: FC<TransferERC20ButtonProps> = ({
    ...
    useEffect(() => {
        try {
            if (contractWrite?.data?.hash) {
                setSwapTransaction(swapId, TransactionStatus.Pending, contractWrite?.data?.hash);
    ...
    const waitForTransaction = useWaitForTransaction({
        ...
        onError: async (err) => {
            if (contractWrite?.data?.hash)
                setSwapTransaction(swapId, TransactionStatus.Failed, contractWrite.data.hash, err.message);
        }
    })
