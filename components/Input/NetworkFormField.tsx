        ...
        const res: SelectMenuItem<RouteNetwork> & { isExchange: boolean } = {
            ...
            subItems: r.tokens.map(token => {
                ...
                const token_select_item: SelectMenuItem<RouteToken & { network_name: string, network_display_name: string, network_logo: string }> = {
                    baseObject: { ...token, network_name: r.name, network_display_name: r.display_name, network_logo: r.logo },
                    id: `${token?.symbol?.toLowerCase()}_${r.name?.toLowerCase()}`,
