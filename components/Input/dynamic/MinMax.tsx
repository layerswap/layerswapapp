...
const MinMax = ({ onAddressGet }: { onAddressGet: (address: string) => void }) => {
    ...
    const maxAllowedAmount = calculateMaxAmount({
        maxAmountFromApi,
        query,
        fromCurrency,
        minAllowedAmount,
    ...
    const handleSetMaxAmount = useCallback(async () => {
        const updatedBalances = from && fromCurrency && await fetchBalance(from, fromCurrency);
...
const calculateMaxAmount = ({
    ...
    maxAmountFromApi: number | undefined;
    query: QueryParams;
    fromCurrency: RouteToken | undefined;
    ...
    if (query.balances && fromCurrency) {
        try {
            ...
            if (
                balancesTyped &&
                balancesTyped[fromCurrency.symbol] &&
                balancesTyped[fromCurrency.symbol] > Number(minAllowedAmount)
            ) {
                maxAllowedAmount = Math.min(
                    maxAllowedAmount,
                    balancesTyped[fromCurrency.symbol]
                );
            }
        }
        // in case the query parameter had bad formatting just ignore
        catch { }
    } else if (
        walletBalance &&
        walletBalance.amount >= Number(minAllowedAmount) &&
        walletBalance.amount <= Number(maxAmountFromApi)
    ) {
        if (
            (native_currency?.symbol === fromCurrency?.symbol || !native_currency) &&
            walletBalance.amount - gasAmount >= Number(minAllowedAmount) &&
            walletBalance.amount - gasAmount <= Number(maxAmountFromApi)
        ) {
