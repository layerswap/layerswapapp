...
interface WalletState {
    ...
    addAccount: (v: ParadexAccount) => void;
...
export const useWalletStore = create<WalletState>()(persist((set) => ({
    ...
    addAccount: (value) => set((state) => ({ accounts: { ...state.accounts, ...{ [value.l1Address.toLowerCase()]: value.paradexAddress } } })),
    removeAccount: (value) => set((state) => {
        const { [value.toLowerCase()]: _, ...updatedAccounts } = state.accounts || {};
