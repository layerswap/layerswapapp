...
export class BalanceResolver {
    ...
    async getBalance(network: NetworkWithTokens, address?: string, options?: { timeoutMs?: number }): Promise<NetworkBalance> {
        try {
            if (!address)
            ...
            const balances = await provider.fetchBalance(address, network, options)
