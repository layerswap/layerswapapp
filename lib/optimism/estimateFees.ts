  ...
  export const getL2Client = (options: ClientOptions): PublicClient => {
    ...
    if ('chainId' in options && options.chainId) {
      ...
      return createPublicClient({
        chain: {
          id: typeof options.chainId === 'number' ? options.chainId : options.chainId?.id,
        ...
        transport: http(
          options.rpcUrl ?? chains[typeof options.chainId === 'number' ? options.chainId : options.chainId?.id].rpcUrls.public.http[0]
        ),
      })
    }
