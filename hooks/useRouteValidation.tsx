...
export function resolveRouteValidation(quoteError?: QuoteError) {
    ...
    if (query?.lockToAsset) {
        if (fromCurrency?.status === 'not_found') {
            ...
            validationDetails = { title: 'Route Unavailable', type: 'warning', icon: <RouteOff className='w-4 h-4 text-warning-primary' /> };
        }
            ...
            validationDetails = { title: 'Temporarily unavailable.', type: 'warning', icon: <Info className='w-4 h-4 text-warning-primary' /> };
        }
        else if (!toCurrency) {
    ...
    if (((from?.name && from?.name.toLowerCase() === query.sameAccountNetwork?.toLowerCase()) || (to?.name && to?.name.toLowerCase() === query.sameAccountNetwork?.toLowerCase()))) {
        ...
        if ((selectedSourceAccount && destination_address && selectedSourceAccount?.address?.toLowerCase() !== destination_address?.toLowerCase())) {
            ...
            validationDetails = { title: 'Action Needed', type: 'warning', icon: <RouteOff className='w-4 h-4 text-warning-primary' /> };
        }
        ...
        if (values.depositMethod === "deposit_address") {
            ...
            validationDetails = { title: 'Manual Transfer is not supported', type: 'warning', icon: <RouteOff className='w-4 h-4 text-warning-primary' /> };
        }
    ...
    if (quoteError) {
        validationMessage = '';
        validationDetails = { title: quoteMessage || 'Unable to retrieve quote', type: 'warning', icon: <RouteOff className='w-4 h-4 text-warning-primary' /> };
    }
