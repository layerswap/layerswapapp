                                            ...
                                            <table className="min-w-full divide-y divide-darkblue-500 ">
                                                <thead className="bg-darkblue-50">
                                                    <tr className="divide-x divide-darkblue-500">
                                                        ...
                                                        {
                                                            props?.networks?.map((n, index) => (
                                                                <th key={index} scope="col" className="px-4 py-3.5 text-left text-sm font-semibold text-primary-text">
                                                                    {n.display_name}
                                                                </th>
                                                            ))
                                                        }
                                                ...
                                                <tbody className="divide-y divide-darkblue-600 bg-darkblue-300">
                                                    {props?.exchanges.map((e) => (
                                                        <tr key={e.id} className="divide-x divide-darkblue-500">
                                                            ...
                                                            {props?.networks?.map((n, index) => (
                                                                <td key={index} className="whitespace-nowrap p-4 text-sm text-white">{e?.currencies.map((currency) => currency.asset).filter(e => n?.currencies.map((c) => c.asset).includes(e)).filter((v, i, a) => a.indexOf(v) === i).join(', ')}</td>
                                                            ))}
                                                        </tr>
                                                    ))}
                                                </tbody>
